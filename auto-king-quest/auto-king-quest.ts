...
import { BaseScript, GameRect, ocrByClipboard, screenCapture } from '../modules/common';
...
export class AutoKingQuest extends BaseScript {
    ...
    async getKingQuest() {
        try {
            await this.bttService.mouseMoveToXY(0, 0);
            await uSleep(100);
            ...
            // 퀘스트 받기 시도
            await uSleep(100);
            await this.bttService.mouseLeftClick();
            await uSleep(100);
            ...
            await this.bttService.sendKey(BttKeyCode.ESC, 2000);
        }
    }
    ...
    async processRequestQuest() {
        ...
        await this.bttService.sendKey(BttKeyCode.ESC, 250);
    ...
    async processRejectQuest() {
        ...
        await this.bttService.sendKey(BttKeyCode.Enter, 100);

        return;
    }

    async teleport() {
        await this.bttService.sendKey(BttKeyCode.Number0);
        // 출두 캐릭

        const gameLog = await this.getLastGameLog();
    ...
    private async extractGameBoxText() {
        await screenCapture({ rect: this.activeWindowRect });
